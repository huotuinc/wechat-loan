<template>
  <div>
    ...
    <group title="主要联系人信息(需直系亲属)">
      ...
      <x-input title="姓名" ref="name" required :show-clear="false" v-model.trim="form[0].name"></x-input>
      <x-input
        title="手机号"
        name="mobile"
        keyboard="number"
    ...
    <group title="其他联系人信息">
      <popup-picker title="关系" :data="list"
                    v-model="value1"
                    popup-title="关系"></popup-picker>
      <x-input title="姓名" ref="name" required :show-clear="false" v-model.trim="form[1].name"></x-input>
      <x-input
        name="mobile"
        title="手机号"
        keyboard="number"
  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        ...
        this.$store
          .dispatch('editcontacts', this.form)
          .then(() => {
          //todo 提交成功跳转
